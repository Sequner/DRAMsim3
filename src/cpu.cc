...
void RandomCPU::ClockTick() {
    ...
    if (get_next_) {
        memory_system_.AddTransaction(last_addr_, last_write_, false);
    }
    clk_++;
    return;
}
...
void StreamCPU::ClockTick() {
    ...
    if (!inserted_a_ &&
        ...
        memory_system_.AddTransaction(addr_a_ + offset_, false, false);
        inserted_a_ = true;
    }
    if (!inserted_b_ &&
        memory_system_.WillAcceptTransaction(addr_b_ + offset_, false)) {
        memory_system_.AddTransaction(addr_b_ + offset_, false, false);
        inserted_b_ = true;
    }
    if (!inserted_c_ &&
        memory_system_.WillAcceptTransaction(addr_c_ + offset_, true)) {
        memory_system_.AddTransaction(addr_c_ + offset_, true, false);
        inserted_c_ = true;
    }
    ...
    if (!trace_file_.eof()) {
        ...
        if (trans_.added_cycle <= clk_) {
            get_next_ = memory_system_.WillAcceptTransaction(trans_.addr,
                                                             trans_.is_write);
            if (get_next_) {
                memory_system_.AddTransaction(trans_.addr, trans_.is_write, trans_.is_TRR);
            }
        }
    }
    clk_++;
